...
!       real(RKIND) :: amu, amu_corrected      ! dynamic viscosity [g/s]
       ...
       real(RKIND), dimension( kts:kte) :: rho_col, delz_col_flip
       real(RKIND), dimension(num_chem)    :: dt_settl
...
!          if  (do_timing) call mpas_timer_start('second_K')
             if ( settling_opt .gt. 0 ) then
                ...
                do nv = 1, num_chem
                  ...
                  vsettl = four_ninths * gravity * aerodens * ( growth_fac * ( 0.5_RKIND * dp ))**2.0_RKIND * one_over_dyn_visc
                  dtmax = dzmin / vsettl
                  ndt_settl(nv) = MAX( 1, INT( ntdt /dtmax) )
                  ! Limit maximum number of iterations
                  IF (ndt_settl(nv) > max_iter_settle) ndt_settl(nv) = max_iter_settle
                  dt_settl(nv) = REAL(ntdt,kind=RKIND) /REAL(ndt_settl(nv),kind=RKIND)
                enddo
                ! Perform gravitational settling if desired
                call particle_settling(cblk_col,rho_col,delz_col_flip,vg_col,dt_settl,ndt_settl,num_chem,kts,kte)
...
!--------------------------------------------------------------------------------
!
subroutine particle_settling(cblk,rho_phy,delz_flip,vg,dt_settl,ndt_settl,num_chem,kts,kte)
     IMPLICIT NONE
...
!--- Local------
     ...
     do k = kte,kts,-1
        ...
        vd_wrk1 = dt_settl(nv) * vg(k,nv)*1.e-2_RKIND / delz_flip(l2)               ! convert vg to m/s
     
        cblk(k,nv)= cblk(k,nv) * (1._RKIND - vd_wrk1) + transfer_to_below_level
        if (k.gt.kts) then
