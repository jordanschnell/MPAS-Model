...
!       real(RKIND) :: amu, amu_corrected      ! dynamic viscosity [g/s]
       ...
       real(RKIND), dimension( kts:kte) :: rho_col, delz_col_flip, cblk_col, cblk_col_before,vg_col
       real(RKIND) :: dt_settl
...
!> -- Gas constant
       ...
       do nv = 1, num_chem
          ...
          do i = its, ite
             ...
             ! depvel uses meters, need to convert to s/cm (done in depvel subr)
             ...
             ! Set if snow greater than 1 cm
                ...
                do k = kts, kte
                   ...
                   vg = aerodens * dp * dp * g100 * Cc / &       ! Convert gravity to cm/s^2
                          ( 18._RKIND * airkinvisc * rho_phy(i,k,j) ) ! Convert density to mol/cm^3
                   ...
                   if ( k == kts ) then
                      ! Brownian Diffusion
                      DDp = ( boltzmann * t_phy(i,k,j) ) * Cc / (3._RKIND * pi * airkinvisc * rho_phy(i,k,j)  * dp) ! Convert density to mol/cm^3
                      ! Schmit number
                      Sc = airkinvisc / DDp
                      ! Brownian Diffusion
                      ...
                      drydep_flux(i,j,nv) = cblk_col(k)*rho_phy(i,k,j)*ddvel(i,j,nv)*1.e-2_RKIND*dt*1.e3_RKIND
                   endif ! k == kts
                enddo ! k
